...
![esbuild调研.jpeg](https://p6-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/e02ac5c4f81b49ffbf9af53c791afd2f~tplv-k3u1fbpfcp-jj-mark:0:0:0:0:q75.image#?w=900&h=383&s=162020&e=jpg&b=ffffff)
...
在这个背景下，[esbuild](https://github.com/evanw/esbuild/tree/main) 以惊人的速度和卓越的性能迅速吸引了众多开发者的目光。
...
近两年来，[esbuild](https://github.com/evanw/esbuild/tree/main) 的 npm 下载量也呈现出迅猛增长的趋势，一副要和老大哥 [Webpack](https://www.npmjs.com/package/webpack) 争一争这头把交椅的味道。

---

在一些开发者眼中，[esbuild](https://github.com/evanw/esbuild/tree/main) 不仅是一个构建工具，更像是一个革命先驱。它对现代前端构建范式发起了一场革命，让我们突然意识到，前端工具的选择并非局限于 `Node.js`，还包括像 `Golang`、`Rust` 这样的新兴技术栈。

本文将从实际需求出发，深入调研 [esbuild](https://github.com/evanw/esbuild/tree/main) 的应用场景。和大家探讨一下：[esbuild](https://github.com/evanw/esbuild/tree/main) 到底是名副其实还是虚有其表？
...
为了解决此问题，社区找了一个插件：[@craftamap/esbuild-plugin-html](https://www.npmjs.com/package/@craftamap/esbuild-plugin-html)。它可以帮我们提供 **HTML 模版**，并能将打包后的文件自动插入到 **HTML 模版** 中，类型于 **Webpack** 的 [webpack-html-template](https://www.npmjs.com/package/webpack-html-template)。
...
> 小结：在 `css`、`js`、`html` 中引入图片成功。但 **esbuild** 仅支持在 `css`、`js` 中引用图片资源。如果想要在 `.html` 中正常显示，需要 [@craftamap/esbuild-plugin-html](https://www.npmjs.com/package/@craftamap/esbuild-plugin-html) 插件和手写相应的复制文件插件来完成。
...
- [tsx](https://esbuild.github.io/content-types/#typescript)：处理 .tsx 文件，不多说。
- [css](https://esbuild.github.io/content-types/#css)：处理 .css 文件，不多说。
...
然后在 [esbuild-plugin-html](https://www.npmjs.com/package/esbuild-plugin-html) 插件中配置我们需要加载的 cdn 链接：
...
- [@babel/core](https://www.npmjs.com/package/@babel/core)：Babel 编译器的核心，用于转译新版 JavaScript 代码。
- [@babel/preset-env](https://www.npmjs.com/package/@babel/preset-env)：Babel 的预设，根据目标环境智能选择转译插件。
- [@babel/preset-react](https://www.npmjs.com/package/@babel/preset-react)：用于处理 React 中的 JSX 语法转译。
- [@babel/preset-typescript](https://www.npmjs.com/package/@babel/preset-typescript)：用于处理 TypeScript 代码转译。
...
10. [【万字长文｜趣味图解】彻底弄懂 Webpack 中的 Loader 机制](https://juejin.cn/post/7157739406835580965)

...
这个时候单独配置 esbuild 的 target 属性是不会生效的，我们还是需要借助于 postcss，这里尝试了几个 postcss 插件均失败。这里自己写一个简易的 postcss 插件：xxx。

配置上我们的插件：浏览器加前缀插件和解决 var 变量的插件。配置完成后重新打包，这是打包前的代码 xx，这是打包后的代码 xx。

这里有个问题，那就是 postcss 一般是需要配置需要兼容的浏览器版本的，所以 esbuild 的 target 属性并不会在 post-css 中生效。因此我们还是需要建立一个.browserslistrc 配置文件来高速 post-css 我们需要兼容哪些浏览器。
...
- 为了更持久的使用缓存，并能够及时的请求最新文件，我们一般会给输出文件加 hash，并对 按照打包后的文件类型进行分类。比如资源文件放在 assets 中，js 放在 js 中，css 放在 css 文件夹中。配置：
  // 对资源文件分类和加内容 hash
